...
PYBIND11_MODULE(Pybind11Test, m) {
    ...
    public:
        ...
        float operator()(ssize_t i, ssize_t j) const { return m_data[(size_t)(i * m_cols + j)]; }

        float &operator()(ssize_t i, ssize_t j) { return m_data[(size_t)(i * m_cols + j)]; }
    ...
    py::class_<Matrix>(m, "Matrix", py::buffer_protocol())
        ...
        .def("__getitem__",
             [](const Matrix &m, std::pair<ssize_t, ssize_t> i) {
                 if (i.first >= m.rows() || i.second >= m.cols())
                     throw py::index_error();
                 return m(i.first, i.second);
             })
        .def("__setitem__",
             [](Matrix &m, std::pair<ssize_t, ssize_t> i, float v) {
                 if (i.first >= m.rows() || i.second >= m.cols())
                     throw py::index_error();
    ...
    py::class_<SquareMatrix, Matrix>(m, "SquareMatrix").def(py::init<ssize_t>());
